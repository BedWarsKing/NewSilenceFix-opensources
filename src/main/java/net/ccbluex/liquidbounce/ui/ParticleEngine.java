...
public class ParticleEngine {
    ...
    public void render(float mouseX, float mouseY){
        ...
        for(Particle p : particles){
            ...
            MiLiBlueUtil.drawBorderedCircle(p.x + Math.sin(p.ticks/2)*50 + -xOffset/5, (p.ticks*p.speed)*p.ticks/10 + -yOffset/5, p.radius*(p.opacity/32), c.getRGB(), c.getRGB());
        ...
        GlStateManager.enableBlend();
        GlStateManager.disableAlpha();
        lastMouseX = MiLiBlueUtil.getMouseX();
        lastMouseY = MiLiBlueUtil.getMouseY();
    }
}
