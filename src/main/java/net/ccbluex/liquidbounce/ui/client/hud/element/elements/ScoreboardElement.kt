...
class ScoreboardElement(
    ...
    private val strokeredValue = IntegerValue("StrokeRed", 255, 0, 255)
    private val strokegreenValue = IntegerValue("StrokeGreen", 255, 0, 255)
    private val strokeblueValue = IntegerValue("StrokeBlue", 255, 0, 255)
    private val bgmodValue = ListValue("Theme", arrayOf("1", "2", "3"), "Back")
    private val bgAlphaValue = IntegerValue("ThemeAlpha", 120, 0, 255)
    // val stroke = BoolValue("Stroke", false)
    ...
    override fun drawElement(partialTicks: Float): Border? {
        ...
        val strokeColor = strokeColor().rgb
        ...
        val l1 = -maxWidth - 3
        val bg = when(bgmodValue.get()) {
            "1" -> Color(0, 0, 0, bgAlphaValue.get()).rgb
            "2" -> Color(255, 255, 255, bgAlphaValue.get()).rgb
            else -> Color(0, 0, 27, bgAlphaValue.get()).rgb
        }

        if(!MovementUtils.isMoving()){
            when(nomoveValue.get()){
                "Hide" -> return null
                "Text" -> {
                    Fonts.font35.drawString("b", (l1 / 2F) - (fontRenderer.getStringWidth("Scoreboard") * 0.5F) - 1F, -12F, Color(0,0,0).rgb, false)
                }
            }
        }

        RenderUtils.drawCircleRect(l1 - 3F, -13F, 6F, maxHeight + fontRenderer.FONT_HEIGHT + 1.0F, 3F,bg)
        val w2 = (l1 / 2F) - (fontRenderer.getStringWidth("Scoreboard") * 0.5F)

        fontRenderer.drawString("Scoreboard", w2 + 0.5F, -12F, Color(0,0,0).rgb, true)
        fontRenderer.drawString("Scoreboard", w2 - 0.5F, -12F, Color(0,0,0).rgb, true)
        fontRenderer.drawString("Scoreboard", w2, -12F + 0.5F, Color(0,0,0).rgb, true)
        fontRenderer.drawString("Scoreboard", w2, -12F - 0.5F, Color(0,0,0).rgb, true)

        fontRenderer.drawString("Scoreboard", w2, -12F, Color(255,255,255).rgb, true)
        ...
        scoreCollection.forEachIndexed { index, score ->
            ...
            if (!noPointValue.get()) {
                fontRenderer.drawString(scorePoints, (width - fontRenderer.getStringWidth(scorePoints)).toFloat(), height.toFloat(), Color.WHITE.rgb, shadowValue.get())
            }

            if (index == scoreCollection.size - 1) {
                val displayName = objective.displayName
                GlStateManager.resetColor()
                fontRenderer.drawString(displayName, (l1 + maxWidth / 2 - fontRenderer.getStringWidth(displayName) / 2).toFloat(), (height - fontRenderer.FONT_HEIGHT).toFloat(), textColor, shadowValue.get())
            }
        }

        return Border(-maxWidth.toFloat() - 6, -13F, 6F, maxHeight.toFloat() + fontRenderer.FONT_HEIGHT + 1)
    }

    private fun textColor() = Color(textRedValue.get(), textGreenValue.get(), textBlueValue.get())
    private fun strokeColor() = Color(strokeredValue.get(), strokegreenValue.get(), strokeblueValue.get())
}