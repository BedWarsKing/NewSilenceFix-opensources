...
class NoFall : Module() {
    ...
    private val noVoid = BoolValue("NoVoid", true)
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (!PlayerUtils.checkVoid()) {
            canNoFall = true
        }

        if (noVoid.get() && PlayerUtils.checkVoid() && mc.thePlayer.fallDistance > 4) {
            canNoFall = false
            return
        }
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (event.packet is C03PacketPlayer) {
            ...
            if (mode.equals("SpoofGround", ignoreCase = true) && mc.thePlayer.fallDistance > 2.5) {
                ...
                packet.onGround = true
                matrixCanSpoof = false
            } else if (mode.equals("NewHypixel", true)) {
                if (!mc.thePlayer.capabilities.isFlying && !mc.thePlayer.capabilities.disableDamage && mc.thePlayer.motionY < 0.0 && packet.isMoving && mc.thePlayer.fallDistance > 2.0f) {
                    event.cancelEvent()
                    sendPacketNoEvent(
                        C04PacketPlayerPosition(
                            packet.x,
    ...
    fun onMove(event: MoveEvent) {
        ...
        } else if (modeValue.equals("NewHypixel")) {
            if (!mc.thePlayer.capabilities.isFlying && !mc.thePlayer.capabilities.disableDamage && mc.thePlayer.motionY < 0.0 && mc.thePlayer.fallDistance > 3.0f) {
